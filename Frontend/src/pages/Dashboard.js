...
export default function Dashboard({ authToken, setPage }) {
  ...
  const [currentPath, setCurrentPath] = useState(() => {
    // Initialize from localStorage, but only if it exists
  ...
  // Clear search and filters when path changes
  useEffect(() => {
    setSearchQuery("");
    setSearchResults([]);
    setYearFilter("");
  ...
  const navigateToFolder = (folderPath) => {
    if (searchQuery) {
      // When coming from search results, use the full path
      setCurrentPath(folderPath);
      setSearchQuery("");
      setSearchResults([]);
    } else {
      // For normal navigation, append to current path
  ...
  const handleFileClick = async (item) => {
    if (item.type === "folder") {
      ...
      setIsLoading(true);
      try {
        const filePath = searchQuery ? item.path : (currentPath ? `${currentPath}/${item.name}` : item.name);
